...
!function( $ ) {
    ...
    Datepicker.prototype = {
        ...
        hide: function(e){
            ...
            if (e && e.currentTarget.value && this.clickCount === 0) {
                this.setValue();
            }
            else {
                this.clickCount = 0;
        ...
        setValue: function() {
            ...
            if (this.range) {
                formatted = DPGlobal.formatDate(this.rangeStart, this.format, this.language) + ' - ' +
                    DPGlobal.formatDate(this.rangeEnd, this.format, this.language);
            }
            if (!this.isInput) {
                if (this.component){
        ...
        update: function(){
            var stringDate = this.isInput ? this.element.prop('value') : this.element.data('date') || this.element.find('input').prop('value') || '';
            this.date = DPGlobal.parseDate(stringDate, this.format, this.language);
            ...
            this.rangeStart = this.rangeEnd = this.date;
            if (this.range) {
                var mats = stringDate.match(/^(\d{4,4})\-(\d{2,2})\-(\d{2,2}) - (\d{4,4})\-(\d{2,2})\-(\d{2,2})$/)
                if (mats) {
                    try {
                        this.rangeStart = new Date(mats[1],mats[2] -1,mats[3],0,0,0,0);
                        this.rangeEnd = new Date(mats[4],mats[5] -1,mats[6],0,0,0,0);
                    }
                    catch (e) {
        ...
        fill: function() {
            ...
            while(prevMonth.valueOf() < nextMonth) {
                ...
                if (this.range) {
                    if (prevMonth >= this.rangeStart && prevMonth <= this.rangeEnd) {
                        clsName += ' active';
                    }
                }
                else {
                            ...
                            if (this.range) {
                                ...
                                if (this.clickCount === 1) {
                                    ...
                                    if (this.rangeEnd < this.rangeStart) {
                                        var tempDate = this.rangeEnd;
                                        this.rangeEnd = this.rangeStart;
                                        this.rangeStart = tempDate;
